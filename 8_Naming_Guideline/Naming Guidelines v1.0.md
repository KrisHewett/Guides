...
**Audience:** Data engineers, analytics developers, data stewards, report authors
**Scope:** Relational databases, dimensional models, data marts, and data vault artifacts in Patient Safety
...
2. **Controlled Vocabularies & Codes**
    * Use **suffixes** that make representation explicit:
        * `_id` (surrogate/natural identifier), `_code` (coded domain), `_name` (human-readable label), `_desc` (long description), `_flag` (boolean), `_dt`/`_date` (date), `_ts` (timestamp), `_cnt` (count), `_pct` (percentage), `_amt` (monetary), `_num` (numeric non-count), `_txt` (free text).
...
> Rationale (high-level): ISO/IEC 11179 stresses semantic clarity for data elements, explicit representation forms (codes vs names), and governance through registries and controlled vocabularies. This guide operationalizes those concepts in practical naming conventions.
...
* `is_recurrent_flag`
* `length_of_stay_days` (if applicable contextually)
...
* **Patient:**
  * `patient_id`, `date_of_birth`, `sex_code`, `patient_age_years`, `is_pregnant_flag` (if applicable), `is_vulnerable_flag`
* **Facility:**
...
2. Table names must be ≤ 64 chars; column names ≤ 63 chars (RDBMS-dependent—set stricter internal limits, e.g., 40).
...
* `incident`:
  * `incident_id` (PK)
  * `facility_id` (FK)
  * `incident_severity_code` (FK → `ref_incident_severity`)
  * `incident_status_code` (FK → `ref_incident_status`)
  * `incident_occurrence_dt`
  * `incident_reported_date`
  * `is_recurrent_flag`
  * `created_ts`, `updated_ts`, `source_system_code`, `source_record_id`
...
1. **Ordered workflow**
    1. Propose name → check approved tokens & units → draft ISO/IEC 11179 data element entry.
    2. Data Steward review → registry update → semantic version bump (minor unless directive change).
    3. CI lint passes → deployment artifact generated → audit log entry created.
...
> Per your directive to **always** display **full absolute path names** for artifact references in storage guidance tables:
