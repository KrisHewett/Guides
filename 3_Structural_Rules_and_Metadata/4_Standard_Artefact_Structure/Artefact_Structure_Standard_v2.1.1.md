...
Conforms To Directive: MCM Artefact Structure Standard v2.1.1

SSOT Path: \\Guides\3_Structural_Rules_and_Metadata\4_Standard_Artefact_Structure\Artefact_Structure_Standard_v2.1.1.md
...
artefacts under the Master Compliance Memory (MCM). Version 2.1.1 introduces metadata termination rules, the
Blank‑Line Metadata Block, and prohibits fenced code blocks.
...
- Artefacts must not contain commentary, reasoning, diagnostics, validation output, lint results, or placeholder text.
...
- `<email>` and `<<email>>` formats are both permitted; `<email>` is preferred.
- SSOT path must match filename and version exactly.
- The metadata block must end with:
  1) a blank line, and
  2) a horizontal rule (`---`)
- A blank line is mandatory before the terminating horizontal rule.
- Fenced code blocks are prohibited.
- Code examples must use 4‑space indentation.
...
- MD040 — 4‑space indentation where code is used
- MD033 — inline HTML prohibited except for email addressing
...
A versioning table must appear as follows:
...
May be included only when explicitly requested and must follow:
...
- Markdownlint Ruleset (MD025, MD032, MD035, MD040, MD033, MD060)
...
| Version | Date       | Author                         | Changes                                                                 |
|--------:|------------|--------------------------------|--------------------------------------------------------------------------|
| 2.1.1   | 2026‑02‑27 | <Kristopher.Hewett@sa.gov.au>  | Prohibited fenced code blocks; added metadata termination rules.         |
| 2.1.0   | 2026‑02‑27 | <Kristopher.Hewett@sa.gov.au>  | Introduced Blank‑Line Metadata Block; replaced “Developed by:” with “Sponsor:”. |
